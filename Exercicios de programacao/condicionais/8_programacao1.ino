...
void loop()
{
 ...
 Serial.println("Digite o nome do funcionario");//Imprime uma mensagem na tela
  while(!Serial.available());//pausa o progarama ate o usuario teclar (Enter>
  ...
  while(!Serial.available());//pausa o progarama ate o usuario teclar (Enter>
  cargo = Serial. parseFloat();//guarda o texto digitado na variavel 
  ...
  if(salario < 1000 ){
    //calcular o aumento
    salario = salario + (salario * 0.10);//calcular 10% no salario
 ...
 Serial.println("Funcinario:" + nome);//imprimir o nome 
